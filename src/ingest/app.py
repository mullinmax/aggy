...
def build_feed_to_ingest_list():
    ...
    for user_hash in user_hashes:
        ...
        if len(feed_hashes) > 0:
            target_time = (datetime.now() + timedelta(seconds=config.get('FEED_INGESTION_PERIOD'))).timestamp()
            r.zadd(
...
def parse_next_feed():
    ...
    r.zadd(
        config.get('FEEDS_TO_INGEST_KEY'), 
        mapping={feed_key: (datetime.now() + timedelta(seconds=config.get('FEED_INGESTION_PERIOD'))).timestamp()},
        lt=True #take the sooner of the values if a feed already exists
    )
