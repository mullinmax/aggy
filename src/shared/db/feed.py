...
class Feed(BlinderBaseModel):
    ...
    def _add_to_category(self, category_hash):
        with self.redis_con() as r:
            r.sadd(
                f"USER:{self.user_hash}:CATEGORY:{category_hash}:FEEDS", self.name_hash
            )
            r.sadd(
                f"USER:{self.user_hash}:FEED:{self.name_hash}:CATEGORIES", category_hash
            )
    ...
    def update_categories(self, new_category_hashes: Set[str]):
        ...
        with self.redis_con() as r:
            ...
            current_categories = r.smembers(
                f"USER:{self.user_hash}:FEED:{self.name_hash}:CATEGORIES"
            )
            ...
            for category_hash in categories_to_remove:
                r.srem(
                    f"USER:{self.user_hash}:CATEGORY:{category_hash}:FEEDS",
                    self.name_hash,
                )
                r.srem(
                    f"USER:{self.user_hash}:FEED:{self.name_hash}:CATEGORIES",
    ...
    @classmethod
    def read(cls, user_hash, feed_hash) -> "Feed":
        feed_key = f"USER:{user_hash}:FEED:{feed_hash}"
        with cls.redis_con() as r:
            if r.exists(feed_key):
                feed_data = r.hgetall(feed_key)
                category_hashes = r.smembers(
                    f"USER:{user_hash}:FEED:{feed_hash}:CATEGORIES"
                )
                return Feed(
                    **feed_data, category_hashes=category_hashes, name_hash=feed_hash
                )
            else:
                return None
    ...
    def read_all(cls, user_hash) -> List["Feed"]:
        with cls.redis_con() as r:
            ...
            for feed_hash in feed_hashes:
                ...
                feed_data["category_hashes"] = r.smembers(
                    f"USER:{user_hash}:FEED:{feed_hash}:CATEGORIES"
                )
